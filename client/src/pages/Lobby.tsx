...
import { Users, Timer, Trophy, Play, Plus, Loader2, LogOut, Trash2, Copy, Check } from "lucide-react";
...
export default function Lobby() {
  ...
  const handleAddPlayer = (e: React.FormEvent) => {
    ...
    createPlayer.mutate(
      { name: newPlayerName, isActive: true, isGoalkeeper: isGoalkeeper },
      {
        ...
        onError: (err: any) => {
          toast({ title: "Erro ao adicionar jogador", description: err.message, variant: "destructive" });
        }
      },
    );
  };
  ...
  if (isLoading) {
    return <div className="flex h-screen items-center justify-center bg-background text-primary"><Loader2 className="w-12 h-12 animate-spin" /></div>;
  }
    ...
    <div className="min-h-screen bg-background p-4 md:p-8 max-w-5xl mx-auto space-y-8 pb-24">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div className="space-y-1">
          <h1 className="text-4xl md:text-6xl font-black text-primary uppercase tracking-tighter">{session?.name || "FUT SHAMPIONS"}</h1>
          <div className="flex items-center gap-3 bg-muted/50 px-3 py-1.5 rounded-lg border border-border/50">
            ...
            <button onClick={copyLink} className="p-1 hover:bg-muted rounded-md transition-colors text-muted-foreground hover:text-primary">
              {copied ? <Check className="w-4 h-4 text-green-500" /> : <Copy className="w-4 h-4" />}
            </button>
          </div>
        </div>
        <div className="flex gap-2">
          <ShinyButton variant="ghost" size="sm" onClick={handleLogout} className="text-muted-foreground"><LogOut className="w-4 h-4 mr-2" /> Sair</ShinyButton>
          <ShinyButton variant="danger" size="sm" onClick={() => { if (confirm("Deseja deletar esta pelada?")) deleteSession.mutate(); }}><Trash2 className="w-4 h-4 mr-2" /> Deletar</ShinyButton>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="space-y-6">
          <div className="bg-card rounded-2xl p-6 shadow-sm border border-border/50 space-y-6">
            <h2 className="flex items-center gap-2 text-2xl"><Trophy className="w-6 h-6 text-secondary" /> Regras</h2>
            <div className="space-y-4">
              <div className="space-y-2"><Label htmlFor="playersPerTeam">Jogadores por equipe</Label><Input id="playersPerTeam" type="number" min={1} value={state.settings.playersPerTeam} onChange={(e) => updateSettings({ playersPerTeam: parseInt(e.target.value) || 1 })} /></div>
              ...
              {state.settings.winCondition === "goals" && (
                <div className="space-y-2 pt-2"><Label htmlFor="goalsToWin">Gols para vitória</Label><Input id="goalsToWin" type="number" min={1} value={state.settings.goalsToWin} onChange={(e) => updateSettings({ goalsToWin: parseInt(e.target.value) || 2 })} /></div>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="bg-card rounded-2xl p-6 shadow-sm border border-border/50 flex flex-col h-full min-h-[500px]">
            <h2 className="text-2xl mb-6 flex items-center gap-2"><Users className="w-6 h-6 text-accent" /> Lista ({players?.length || 0})</h2>
            <form onSubmit={handleAddPlayer} className="flex flex-col gap-4 mb-6">
              <div className="flex gap-2"><Input value={newPlayerName} onChange={(e) => setNewPlayerName(e.target.value)} placeholder="Nome do jogador..." className="flex-1" /><ShinyButton type="submit" size="sm" disabled={createPlayer.isPending || !newPlayerName}><Plus className="w-5 h-5" /></ShinyButton></div>
              <div className="flex items-center gap-2 px-1"><Checkbox id="isGoalkeeper" checked={isGoalkeeper} onCheckedChange={(c) => setIsGoalkeeper(!!c)} /><Label htmlFor="isGoalkeeper" className="text-sm font-medium leading-none cursor-pointer">Goleiro Fixo</Label></div>
            </form>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 overflow-y-auto max-h-[400px] pr-2">
              {players?.map((player) => (
                <PlayerCard key={player.id} player={player} onRemove={() => deletePlayer.mutate(player.id)} />
              ))}
            </div>
          </div>
        </div>
      ...
      <div className="fixed bottom-6 left-0 right-0 px-4 flex justify-center z-50">
        <ShinyButton size="lg" onClick={handleStartGame} disabled={!players || players.length < 2} className="w-full max-w-md shadow-2xl text-xl">INICIAR PARTIDA <Play className="w-6 h-6 ml-2 fill-current" /></ShinyButton>
      </div>
    </div>
  );
}
